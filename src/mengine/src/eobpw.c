...
// ============================
void eopbend_wilson(int nang,int **i13,int *use,double *x,double *y,double *z,int *angin,float *copb,double *eopb)
{
      int i,ia,ib,ic,id;
...
// ===============================
void eopbend_wilson1(int natom,int nang,int **i13,int *use,double *x,double *y,double *z,int *angin,float *copb,double *eopb,double **deopb)
{
      int i,ia,ib,ic,id;
      double e,angle,dot,cosine;
      double dt,dt2;
              ...
              if (use[ia] || use[ib] || use[ic] || use[id])
              {
                 ...
                 if (rab2 != 0.0 && rbc2 != 0.0)
                 {
                    ...
                    *eopb += e;
                    ...
                    deddt = 2.0*units.angunit*copb[i]*dt;
                    deddt *= radian;
                    ...
                    deopb[ia][0] += deddt*dedxia;
                    deopb[ia][1] += deddt*dedyia;
                    deopb[ia][2] += deddt*dedzia;

                    deopb[ib][0] += deddt*dedxib;
                    deopb[ib][1] += deddt*dedyib;
                    deopb[ib][2] += deddt*dedzib;
                    
                    deopb[ic][0] += deddt*dedxic;
                    deopb[ic][1] += deddt*dedyic;
                    deopb[ic][2] += deddt*dedzic;
                    
                    deopb[id][0] += deddt*dedxid;
                    deopb[id][1] += deddt*dedyid;
                    deopb[id][2] += deddt*dedzid;
...
// ================================       
void eopbend_wilson2(int iatom,int nang,int **i13,int *use,double *x,double *y,double *z,int *angin,float *copb,
             float **hessx,float **hessy,float **hessz)
{
      int i,ia,ib,ic,id;
      double e,angle,dot,cosine;
      ...
      for (i=0; i < nang; i++)
      {
          if (angin[i] == TRUE)
          {
              ...
              if (iatom == ia || iatom == ib || iatom == ic || iatom == id)
              {
                 ...
                 if (rab2 != 0.0 && rbc2 != 0.0)
                 {
                    ...
                    deddt = 2.0*units.angunit*copb[i]*radian*radian;
         ...
         if (iatom == ia)
         {
            hessx[ia][0] += dedxiaxia ;
            hessx[ia][1] += dedxiayia ;
            hessx[ia][2] += dedxiazia ;
            
            hessy[ia][0] += dedxiayia ;            
            hessy[ia][1] += dedyiayia ;
            hessy[ia][2] += dedyiazia ;
            
            hessz[ia][0] += dedxiazia ;
            hessz[ia][1] += dedyiazia ;            
            hessz[ia][2] += dedziazia ;
                       
            hessx[ib][0] += dedxiaxib ;
            hessx[ib][1] += dedxiayib ;
            hessx[ib][2] += dedxiazib ;
            
            hessy[ib][0] += dedxibyia ;            
            hessy[ib][1] += dedyiayib ;
            hessy[ib][2] += dedyiazib ;
            
            hessz[ib][0] += dedxibzia ;
            hessz[ib][1] += dedyibzia ;            
            hessz[ib][2] += dedziazib ;
                      
            hessx[ic][0] += dedxiaxic ;
            hessy[ic][0] += dedxicyia ;
            hessz[ic][0] += dedxiczia ;
                        
            hessx[ic][1] += dedxiayic ;
            hessy[ic][1] += dedyiayic ;
            hessz[ic][1] += dedyiczia ;
            
            hessx[ic][2] += dedxiazic ;
            hessy[ic][2] += dedyiazic ;
            hessz[ic][2] += dedziazic ;
            
            hessx[id][0] += dedxiaxid ;
            hessy[id][0] += dedxidyia ;
            hessz[id][0] += dedxidzia ;
            hessx[id][1] += dedxiayid ;
            hessy[id][1] += dedyiayid ;
            hessz[id][1] += dedyidzia ;
            hessx[id][2] += dedxiazid ;
            hessy[id][2] += dedyiazid ;
            hessz[id][2] += dedziazid ;
         }else if (iatom == ib)
         {
            hessx[ib][0] += dedxibxib ;
            hessy[ib][0] += dedxibyib ;
            hessz[ib][0] += dedxibzib ;           
            hessx[ib][1] += dedxibyib ;
            hessy[ib][1] += dedyibyib ;
            hessz[ib][1] += dedyibzib ;           
            hessx[ib][2] += dedxibzib ;
            hessy[ib][2] += dedyibzib ;
            hessz[ib][2] += dedzibzib ;
                       
            hessx[ia][0] += dedxiaxib ;
            hessy[ia][0] += dedxiayib ;
            hessz[ia][0] += dedxiazib ;            
            hessx[ia][1] += dedxibyia ;
            hessy[ia][1] += dedyiayib ;
            hessz[ia][1] += dedyiazib ;            
            hessx[ia][2] += dedxibzia ;
            hessy[ia][2] += dedyibzia ;
            hessz[ia][2] += dedziazib ;
                      
            hessx[ic][0] += dedxibxic ;
            hessy[ic][0] += dedxicyib ;
            hessz[ic][0] += dedxiczib ;
                      
            hessx[ic][1] += dedxibyic ;
            hessy[ic][1] += dedyibyic ;
            hessz[ic][1] += dedyiczib ;
                     
            hessx[ic][2] += dedxibzic ;
            hessy[ic][2] += dedyibzic ;
            hessz[ic][2] += dedzibzic ;
                     
            hessx[id][0] += dedxibxid ;
            hessy[id][0] += dedxidyib ;
            hessz[id][0] += dedxidzib ;
                       
            hessx[id][1] += dedxibyid ;
            hessy[id][1] += dedyibyid ;
            hessz[id][1] += dedyidzib ;
                      
            hessx[id][2] += dedxibzid ;
            hessy[id][2] += dedyibzid ;
            hessz[id][2] += dedzibzid ;
         }else if (iatom == ic)
         {
            hessx[ic][0] += dedxicxic ;
            hessy[ic][0] += dedxicyic ;
            hessz[ic][0] += dedxiczic ;
            hessx[ic][1] += dedxicyic ;
            hessy[ic][1] += dedyicyic ;
            hessz[ic][1] += dedyiczic ;
            hessx[ic][2] += dedxiczic ;
            hessy[ic][2] += dedyiczic ;
            hessz[ic][2] += dedziczic ;
            hessx[ia][0] += dedxiaxic ;
            hessy[ia][0] += dedxiayic ;
            hessz[ia][0] += dedxiazic ;
            hessx[ia][1] += dedxicyia ;
            hessy[ia][1] += dedyiayic ;
            hessz[ia][1] += dedyiazic ;
            hessx[ia][2] += dedxiczia ;
            hessy[ia][2] += dedyiczia ;
            hessz[ia][2] += dedziazic ;
            hessx[ib][0] += dedxibxic ;
            hessy[ib][0] += dedxibyic ;
            hessz[ib][0] += dedxibzic ;
            hessx[ib][1] += dedxicyib ;
            hessy[ib][1] += dedyibyic ;
            hessz[ib][1] += dedyibzic ;            
            hessx[ib][2] += dedxiczib ;
            hessy[ib][2] += dedyiczib ;
            hessz[ib][2] += dedzibzic ;            
            hessx[id][0] += dedxicxid ;
            hessy[id][0] += dedxidyic ;
            hessz[id][0] += dedxidzic ;            
            hessx[id][1] += dedxicyid ;
            hessy[id][1] += dedyicyid ;
            hessz[id][1] += dedyidzic ;            
            hessx[id][2] += dedxiczid ;
            hessy[id][2] += dedyiczid ;
            hessz[id][2] += dedziczid ;
         }else if (iatom == id)
         {
            hessx[id][0] += dedxidxid ;
            hessy[id][0] += dedxidyid ;
            hessz[id][0] += dedxidzid ;
            
            hessx[id][1] += dedxidyid ;
            hessy[id][1] += dedyidyid ;
            hessz[id][1] += dedyidzid ;
            
            hessx[id][2] += dedxidzid ;
            hessy[id][2] += dedyidzid ;
            hessz[id][2] += dedzidzid ;
            
            hessx[ia][0] += dedxiaxid ;
            hessy[ia][0] += dedxiayid ;
            hessz[ia][0] += dedxiazid ;
            hessx[ia][1] += dedxidyia ;
            hessy[ia][1] += dedyiayid ;
            hessz[ia][1] += dedyiazid ;
            hessx[ia][2] += dedxidzia ;
            hessy[ia][2] += dedyidzia ;
            hessz[ia][2] += dedziazid ;
            
            hessx[ib][0] += dedxibxid ;
            hessy[ib][0] += dedxibyid ;
            hessz[ib][0] += dedxibzid ;
            hessx[ib][1] += dedxidyib ;
            hessy[ib][1] += dedyibyid ;
            hessz[ib][1] += dedyibzid ;
            hessx[ib][2] += dedxidzib ;
            hessy[ib][2] += dedyidzib ;
            hessz[ib][2] += dedzibzid ;
            
            hessx[ic][0] += dedxicxid ;
            hessy[ic][0] += dedxicyid ;
            hessz[ic][0] += dedxiczid ;
            hessx[ic][1] += dedxidyic ;
            hessy[ic][1] += dedyicyid ;
            hessz[ic][1] += dedyiczid ;
            hessx[ic][2] += dedxidzic ;
            hessy[ic][2] += dedyidzic ;
            hessz[ic][2] += dedziczid ;
         }
                 }
