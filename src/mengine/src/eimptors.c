...
void eimptors(int nimptors,int **iiprop,int *use,int *type,double *x,double *y,double *z,float *v1in,float *v2in,float *v3in,int *ph1in,int *ph2in,int *ph3in,double *eimptors);
void eimptors1(int nimptors,int natom,int **iiprop,int *use,int *type,double *x,double *y,double *z,float *v1in,float *v2in,float *v3in,int *ph1in,int *ph2in,int *ph3in,
           double *eimptors,double **deimprop);
...
{
      int i,ia,ib,ic,id;
      double e,cosine,cosine2,cosine3;
        ...
        if (use[ia] || use[ib] || use[ic] || use[id] )
        {
            ...
            if (rtru != 0.0)
            {
               ...
               *eimptors += e;
               if(minim_values.iprint)
               {
                 fprintf(pcmlogfile,"ImpTor:  (%-3d)- (%-3d)- (%-3d)- (%-3d)    %d %d %d %d     %8.3f    %-8.3f %-8.3f %-8.3f = %-8.4f\n",
                   ia,ib,ic,id, type[ia], type[ib],type[ic],type[id], radian*acos(cosine),v1,v2,v3,e);
               }
            }
        }
    }
        ...
        if (use[ia] || use[ib] || use[ic] || use[id] )
        {
            ...
            if (rtru != 0.0)
            {
               ...
               *eimptors += e;
               ...
               deimprop[ia][0] += dedxia;
               deimprop[ia][1] += dedyia;
               deimprop[ia][2] += dedzia;
               deimprop[ib][0] += dedxib;
               deimprop[ib][1] += dedyib;
               deimprop[ib][2] += dedzib;
               deimprop[ic][0] += dedxic;
               deimprop[ic][1] += dedyic;
               deimprop[ic][2] += dedzic;
               deimprop[id][0] += dedxid;
               deimprop[id][1] += dedyid;
               deimprop[id][2] += dedzid;
            }
        }
    }
}
